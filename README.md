...
以用户为单位进行 1,000 次配对 bootstrap 后，ALS 相对热门基线的 Recall@10 绝对变化 95% 区间为 `[+0.0325, +0.0489]`，NDCG@10 为 `[+0.0153, +0.0241]`。两个区间均稳定高于 0，说明这次固定测试中的提升不依赖少数用户。目录覆盖率是推荐集合级指标，仍报告点估计，不套用用户均值置信区间。
...
ALS 的候选召回率 `Recall@100` 为 39.24%。LightGBM 相对 ALS 的 Recall@10 变化 95% 配对 bootstrap 区间为 `[-0.0083, +0.0035]`，NDCG@10 为 `[-0.00679, +0.00002]`；两个区间都跨过 0，因此当前数据既不能证明排序器改善了前 10 名，也不能把单次点估计下降解释为稳定退化。覆盖率点估计增加 4.34 个百分点，但仓库仍不宣称二阶段模型整体优于 ALS。这保留了负结果，同时避免对小差异过度解读。
...
- 排序训练候选包含且仅包含一个验证正样本
- Recall、NDCG 和覆盖率计算
- 用户级配对 bootstrap 的可复现性、参数校验和方向判断
